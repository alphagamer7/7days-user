...
@ViewChild('map', { static: true }) mapElement: ElementRef;
    ...
    lng:any;
    marker: any;
  constructor(private geolocationProvider: Geolocation, private  utilService: UtilService, private ngZone: NgZone,
     private mapService: GoogleMapsService) {
    this.geolocationProvider = geolocationProvider;
    this.utilService = utilService;
    this.ngZone = ngZone;
    this.mapService = mapService;
    this.map_model = { search_query: '', search_places_predictions: [], using_geolocation: false };
...
geocodePosition(pos) {
    ...
    this.geocoder.geocode({
        ...
        if (status == google.maps.GeocoderStatus.OK) {
            console.log(results);
            this.mapService.geocodePlace(results[0].place_id).subscribe((place_location) => {
...
searchPlacesPredictions(query) {
    let env = this;
    if (query !== '') {
        this.mapService.getPlacePredictions(query).subscribe((places_predictions) => {
...
selectSearchResult(place) {
    ...
    this.mapService.geocodePlace(place.place_id).subscribe((place_location) => {
        console.log(place_location);
        this.marker.setPosition(place_location);
        this.map.setCenter(this.marker.getPosition());
