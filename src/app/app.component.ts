  ...
  initializeApp() {
    this.platform.ready().then(() => {
      ...
      if (!lng || lng === null) {
        ...
        this.api.post('users/getDefaultSettingsById', param).subscribe(
          (data: any) => {
            console.log('----------------- app setting', data);
            if (data && data.status === 200 && data.data) {
              const manage = data.data.manage;
      ...
      if (uid && uid !== null && uid !== 'null') {
        ...
        this.api.post('users/getById', param).subscribe(
          (data: any) => {
            console.log('user info=>', data);
        ...
        this.api.post('favourite/getByUid', param).subscribe(
          (data: any) => {
      ...
      this.platform.backButton.subscribe(async () => {
        ...
        if (
          this.router.url === '/tabs/categories' ||
          this.router.url === '/tabs/cart' ||
          this.router.url === '/tabs/orders' ||
