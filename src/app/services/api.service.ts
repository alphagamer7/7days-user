...
export class ApiService {
  ...
  createOrderNotification(stores) {
    ...
    ids.forEach((element) => {
      apiCalls.push(this.sendNotification('تلقيت طلب جديد', 'تم استلام الطلب', element));
    });
  ...
  sendNotification(msg, title, id) {
    const body = {
      app_id: '1d544122-7614-4e62-84dd-5f95afa05080', // stores app id
    ...
    const header = {
      headers: new HttpHeaders().set('Content-Type', 'application/json').set('Authorization', `Basic ${environment.onesignal.restKey}`),
    };
  ...
  post(url, body) {
    const header = {
      headers: new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded').set('Basic', `${environment.authToken}`),
    };
  ...
  twilloPost(url, body, sid, token) {
    ...
    const header = {
      headers: new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded').set('Authorization', authorizationData),
    };
    const order = this.JSON_to_URLEncoded(body);
    console.log(order);
    return this.http.post(url, order, header);
